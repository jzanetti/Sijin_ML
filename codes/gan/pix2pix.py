...
parser.add_argument("--img_dir", type=str, default="monet2photo", help="image directory")
...
dataloader = DataLoader(
    ImageDataset(
    os.path.join(opt.img_dir, opt.dataset_name), transforms_=transforms_),
...
val_dataloader = DataLoader(
    ImageDataset(os.path.join(opt.img_dir, opt.dataset_name),transforms_=transforms_, mode="val"),
    batch_size=10,
    shuffle=True,
    # num_workers=1,
    num_workers=0,
...
def sample_images(batches_done):
    ...
    real_A = Variable(imgs["B"].type(Tensor)).to(device)
    real_B = Variable(imgs["A"].type(Tensor)).to(device)
